...
public class ReinforcedBoatEntity extends Entity
{
					...
					for (int s = k; s < l; ++s)
					{
						if (r <= 0 || s != k && s != l - 1)
						{
							...
							if (!(blockState.getBlock() instanceof LilyPadBlock) && VoxelShapes.matchesAnywhere(
									blockState.getCollisionShape(this.world, mutable).offset((double) p, (double) s,
											(double) q),
	...
	private boolean checkBoatInLava()
	{
		...
		BlockPos.Mutable pooledMutable = new Mutable();
	...
	private void updateVelocity()
	{
		...
		if (this.lastLocation == ReinforcedBoatEntity.Location.IN_AIR
				&& this.location != ReinforcedBoatEntity.Location.IN_AIR
	...
	protected void fall(double heightDifference, boolean onGround, BlockState landedState, BlockPos landedPosition)
	{
		...
		if (!this.hasVehicle())
		{
			if (onGround)
			{
				...
				this.fallDistance = 0.0F;
			} else if (!this.world.getFluidState((new BlockPos(getSenseCenterPos())).down()).matches(FluidTags.LAVA)
